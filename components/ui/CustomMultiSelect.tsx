          ...
          {options.map(option => {
            ...
            return (
              <li
                ...
                className={`text-sm ${disabled ? 'cursor-not-allowed text-slate-500' : 'cursor-pointer'} ${selectedValues.includes(option.value) ? 'bg-blue-100' : (disabled ? '' : 'hover:bg-slate-50')}`}
                role="option"
                aria-selected={selectedValues.includes(option.value)}
                aria-disabled={disabled}
              >
                {option.tooltip ? <Tooltip text={option.tooltip}>{content}</Tooltip> : content}
              </li>
            )
          })}
        </ul>
