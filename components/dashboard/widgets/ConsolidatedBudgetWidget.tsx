...
const ConsolidatedBudgetWidget: React.FC<{ isEditMode?: boolean }> = ({ isEditMode }) => {
    ...
    const { totals, grandTotal, consumptionRate } = useMemo(() => {
        ...
        const initial = { requested: 0, approved: 0, committed: 0, validatedPO: 0, completedPV: 0, forecastedPO: 0 };
    ...
    const handleCardClick = () => { if (!isEditMode) navigate('/projects-budget'); };
            ...
            <CardContent className="flex-grow flex items-center justify-center">
                <div className="w-full text-center text-xs sm:text-sm">
                    ...
                    <div className="mt-4 pt-3 border-t-2 border-slate-200">
                        <div className="flex justify-between items-center mb-2"><div className="font-bold text-left text-slate-800 text-xs uppercase tracking-wide flex items-center gap-2">TOTAL GLOBAL<span className="text-[10px] font-normal text-slate-500 normal-case">(Import FDR)</span></div></div>
                        <div className="grid grid-cols-6 gap-2 items-center">
                            ...
                            <div className={`p-2 rounded font-bold text-white border-2 ${globalAvailableBudget >= 0 ? 'bg-green-700 border-green-500' : 'bg-red-700 border-red-500'}`}>{formatCurrency(globalAvailableBudget)}</div>
                        </div>
                    </div>
                </div>
            </CardContent>
