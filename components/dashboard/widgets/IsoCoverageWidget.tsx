...
const IsoCoverageWidget: React.FC<IsoCoverageWidgetProps> = ({ isEditMode, width = 0 }) => {
    ...
    const linkedProjects = useMemo(() => !selectedIsoMeasure ? [] : projects.filter(p => (p.isoMeasures || []).includes(selectedIsoMeasure.code)), [selectedIsoMeasure, projects]);
    const linkedActivities = useMemo(() => !selectedIsoMeasure ? [] : activities.filter(a => a.isoMeasures.includes(selectedIsoMeasure.code)), [selectedIsoMeasure, activities]);
                    ...
                    <div key={chapter} className="mb-4">
                        ...
                        <div className="flex flex-wrap gap-1">
                            {(measures as Omit<IsoMeasure, 'id'>[]).slice().sort((a,b) => a.code.localeCompare(b.code, 'en', { numeric: true })).map((measure) => (
                                ...
                                <div style={{ width: `${boxSize}px`, height: `${boxSize}px` }} className={`flex items-center justify-center rounded font-mono transition-all duration-200 ${getCoverageClasses(measure.code)} ${!isEditMode ? 'cursor-pointer' : ''}`} onClick={(e) => { e.stopPropagation(); handleMeasureClick(measure.code); }}>
            ...
            {selectedIsoMeasure && (
                 <Modal isOpen={!!selectedIsoMeasure} onClose={() => setSelectedIsoMeasure(null)} title={`${selectedIsoMeasure.code} - ${selectedIsoMeasure.title}`}>
                    <div className="space-y-6">
                        ...
                        <div>
                            <h3 className="text-md font-semibold text-slate-800 mb-3 flex items-center gap-2"><ClipboardList size={18} className="text-blue-600"/>Projets liés ({linkedProjects.length})</h3>
                            {linkedProjects.length > 0 ? (
                                <div className="space-y-2 max-h-40 overflow-y-auto">
                                    {linkedProjects.map(project => (
                                        <div key={project.id} onClick={(e) => handleNavigateToProject(e, project.id)} className="flex items-center justify-between p-3 bg-white border border-slate-200 rounded-md hover:bg-blue-50 hover:border-blue-300 cursor-pointer transition-colors group">
                                            <div><span className="font-mono text-xs bg-slate-100 px-2 py-0.5 rounded text-slate-600 mr-2">{project.projectId}</span><span className="text-sm font-medium text-slate-700 group-hover:text-blue-700">{project.title}</span></div>
                        ...
                        <div>
                            <h3 className="text-md font-semibold text-slate-800 mb-3 flex items-center gap-2"><ListChecks size={18} className="text-yellow-600"/>Activités liées ({linkedActivities.length})</h3>
                            {linkedActivities.length > 0 ? (
                                <div className="space-y-2 max-h-40 overflow-y-auto">
                                    {linkedActivities.map(activity => (
                                        <div key={activity.id} onClick={(e) => handleNavigateToActivity(e, activity.id)} className="flex items-center justify-between p-3 bg-white border border-slate-200 rounded-md hover:bg-yellow-50 hover:border-yellow-300 cursor-pointer transition-colors group">
                                            <div><span className="font-mono text-xs bg-slate-100 px-2 py-0.5 rounded text-slate-600 mr-2">{activity.activityId}</span><span className="text-sm font-medium text-slate-700 group-hover:text-yellow-700">{activity.title}</span></div>
