...
import { select, extent, scaleLinear, scaleSqrt, scaleOrdinal, axisBottom, axisLeft, easeCubicOut, schemeTableau10, schemePastel1, schemeBlues } from 'd3';
...
interface BubbleChartProps {
  data: { label: string; xValue: number; yValue: number; sizeValue: number }[];
  config: { 
    measureXLabel: string,
    measureYLabel: string,
    sizeMeasureLabel: string
  };
  colorPalette: ColorPalette;
}

const BubbleChart: React.FC<BubbleChartProps> = ({ data, config, colorPalette }) => {
  ...
  useEffect(() => {
    ...
    const drawChart = () => {
      ...
      const sizeDomain = extent(data, d => d.sizeValue) as [number, number];
      ...
      const radiusScale = scaleSqrt().domain(sizeDomain).range([4, 25]);
        ...
        .on('mouseover', (event, d) => {
            tooltip.style('opacity', .9)
                   .html(`<strong>${d.label}</strong><br/>
                          ...
                          ${config.measureYLabel}: ${d.yValue.toLocaleString('fr-FR')}<br/>
                          ${config.sizeMeasureLabel}: ${d.sizeValue.toLocaleString('fr-FR')}`)
                   .style('left', `${event.pageX + 15}px`)
                   .style('top', `${event.pageY - 28}px`);
            select(event.currentTarget).attr('fill-opacity', 1).attr('stroke-width', 2);
        })
        .on('mouseout', (event) => {
            tooltip.style('opacity', 0);
            select(event.currentTarget).attr('fill-opacity', 0.7).attr('stroke-width', 0);
        })
        .transition()
        .duration(800)
        .ease(easeCubicOut)
