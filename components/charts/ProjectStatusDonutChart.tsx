...
const ProjectStatusDonutChart: React.FC<ProjectStatusDonutChartProps> = ({ data, onSliceClick }) => {
  ...
  useEffect(() => {
    ...
    const drawChart = (width: number, height: number) => {
        ...
        const total = sum(statusData, d => d.count);
  ...
  if (statusData.length === 0) {
    return <div className="flex items-center justify-center h-full text-slate-500">Aucun projet à afficher.</div>;
  }
