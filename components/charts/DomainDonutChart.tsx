import React, { useRef, useEffect, useMemo } from 'react';
// FIX: Replace monolithic d3 import with specific named imports to resolve type errors.
import { select, arc, pie, sum } from 'd3';
import { Activity, SecurityDomain } from '../../types';
import { DOMAIN_BG_COLORS } from '../../constants';

interface DomainDonutChartProps {
  data: Activity[];
  onSliceClick?: (domain: SecurityDomain) => void;
}

const DomainDonutChart: React.FC<DomainDonutChartProps> = ({ data, onSliceClick }) => {
  const svgRef = useRef<SVGSVGElement | null>(null);
  const containerRef = useRef<HTMLDivElement | null>(null);

  const domainData = useMemo(() => {
    const counts = data.reduce((acc, activity) => {
      acc[activity.securityDomain] = (acc[activity.securityDomain] || 0) + 1;
      return acc;
    }, {} as { [key in SecurityDomain]: number });

    return Object.entries(counts).map(([domain, count]) => ({
      domain: domain as SecurityDomain,
      count,
    })).sort((a, b) => a.domain.localeCompare(b.domain));

  }, [data]);

  useEffect(() => {
    if (!svgRef.current || !containerRef.current || domainData.length === 0) return;

    const container = containerRef.current;
    const svg = select(svgRef.current);
    svg.selectAll('*').remove();

    const resizeObserver = new ResizeObserver(entries => {
      for (let entry of entries) {
        const { width, height } = entry.contentRect;
        drawChart(width, height);
      }
    });
    resizeObserver.observe(container);
    
    const drawChart = (width: number, height: number) => {
        svg.selectAll('*').remove();
        const radius = Math.min(width, height) / 2.2;
        const arcGenerator = arc<any>().innerRadius(radius * 0.65).outerRadius(radius);
        const arcHover = arc<any>().innerRadius(radius * 0.65).outerRadius(radius * 1.05);
        const pieGenerator = pie<any>().value(d => d.count).sort(null);

        const g = svg.append('g').attr('transform', `translate(${width / 2}, ${height / 2})`);

        const path = g.selectAll('path')
            .data(pieGenerator(domainData))
            .enter()
            .append('path')
            .attr('d', arcGenerator)
            .attr('fill', d => DOMAIN_BG_COLORS[d.data.domain])
            .attr('stroke', '#fff')
            .style('stroke-width', '2px')
            .style('cursor', 'pointer')
            .on('mouseover', function (event, d) {
                path.transition('fade').duration(200).style('opacity', p => (p === d ? 1 : 0.5));
                select(this).transition('zoom').duration(200).attr('d', arcHover);
                updateCenterText(d.data.domain, d.data.count);
            })
            .on('mouseout', function () {
                path.transition('fade').duration(200).style('opacity', 1);
                select(this).transition('zoom').duration(200).attr('d', arcGenerator);
                resetCenterText();
            })
            .on('click', (event, d) => {
                if (onSliceClick) {
                    onSliceClick(d.data.domain);
                }
            });
        
        const centerText = g.append('text')
            .attr('text-anchor', 'middle')
            .style('font-size', '16px')
            .style('fill', '#475569');

        const centerCount = centerText.append('tspan')
            .attr('x', 0)
            .attr('dy', '-0.1em')
            .style('font-size', '28px')
            .style('font-weight', '600');

        const centerLabel = centerText.append('tspan')
            .attr('x', 0)
            .attr('dy', '1.2em');

        const total = sum(domainData, d => d.count);

        const updateCenterText = (domain: string, count: number) => {
            centerCount.text(count);
            centerLabel.text(domain);
        };
        
        const resetCenterText = () => {
            centerCount.text(total);
            centerLabel.text('Activités');
        };

        resetCenterText();
    }
    
    return () => resizeObserver.disconnect();
  }, [domainData, onSliceClick]);

  if (domainData.length === 0) {
    return <div className="flex items-center justify-center h-full text-slate-500">Aucune donnée de domaine à afficher.</div>;
  }

  return (
    <div className="flex flex-col md:flex-row items-center h-[350px] w-full gap-3">
        <div ref={containerRef} className="flex-1 h-full w-full min-h-0 relative">
            <svg ref={svgRef} width="100%" height="100%"></svg>
        </div>
        <div className="flex-none w-full md:w-48">
            <ul className="space-y-2">
                {domainData.map(({ domain, count }) => (
                    <li key={domain} className="flex items-center text-sm">
                        <span className="w-3 h-3 rounded-full mr-3" style={{ backgroundColor: DOMAIN_BG_COLORS[domain] }}></span>
                        <span className="text-slate-700">{domain}</span>
                        <span className="ml-auto text-slate-600 font-medium">{count}</span>
                    </li>
                ))}
            </ul>
        </div>
    </div>
  );
};

export default DomainDonutChart;
