        ...
        if (sortConfig !== null) {
            sortableItems.sort((a, b) => {
                ...
                if (sortConfig.key === 'totalProgress') {
                    ...
                    aValue = (a.internalWorkloadEngaged || 0) + (a.externalWorkloadEngaged || 0);
                    bValue = (b.internalWorkloadEngaged || 0) + (b.externalWorkloadEngaged || 0);
                } else if (sortConfig.key === 'totalWorkloadConsumed') {
                    aValue = (a.internalWorkloadConsumed || 0) + (a.externalWorkloadConsumed || 0);
                    bValue = (b.internalWorkloadConsumed || 0) + (b.externalWorkloadConsumed || 0);
                }
                 else {
                ...
                <CardContent className="py-4">
                     <div className="flex flex-wrap justify-around items-center gap-4 text-center">
                        <div className="flex flex-col">
                             <span className="text-xs text-slate-500 uppercase font-semibold mb-1">Interne Engagé</span>
                             <span className="text-lg font-bold text-slate-700">{formatJH(totalStats.intEng)}</span>
                        </div>
                        <div className="w-px h-10 bg-slate-200 hidden md:block"></div>
                        <div className="flex flex-col">
                             <span className="text-xs text-slate-500 uppercase font-semibold mb-1">Interne Conso.</span>
                             <span className="text-lg font-bold text-slate-700">{formatJH(totalStats.intCon)}</span>
                        </div>
                        <div className="w-px h-10 bg-slate-200 hidden md:block"></div>
                         <div className="flex flex-col">
                             <span className="text-xs text-slate-500 uppercase font-semibold mb-1">Externe Engagé</span>
                             <span className="text-lg font-bold text-slate-700">{formatJH(totalStats.extEng)}</span>
                        </div>
                        <div className="w-px h-10 bg-slate-200 hidden md:block"></div>
                        <div className="flex flex-col">
                             <span className="text-xs text-slate-500 uppercase font-semibold mb-1">Externe Conso.</span>
                             <span className="text-lg font-bold text-slate-700">{formatJH(totalStats.extCon)}</span>
                        </div>
                        <div className="w-px h-10 bg-slate-200 hidden md:block"></div>
                        ...
                        <table className="w-full text-sm text-left text-slate-500">
                            <thead className="text-xs text-slate-700 uppercase bg-slate-100 sticky top-0">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-center" colSpan={2}>Détail Interne</th>
                                    <th className="px-6 py-3 text-center" colSpan={2}>Détail Externe</th>
                                </tr>
                                <tr className="text-xs bg-slate-100">
                                    <th colSpan={5}></th>
                                    <th className="px-2 py-2 font-medium text-center">Conso.</th>
                                    <th className="px-2 py-2 font-medium text-center">Engagé</th>
                                    <th className="px-2 py-2 font-medium text-center">Conso.</th>
                                    <th className="px-2 py-2 font-medium text-center">Engagé</th>
                                </tr>
                            </thead>
                            <tbody>
                                {sortedProjects.map(project => {
                                    ...
                                    <tr key={project.id} className="bg-white border-b hover:bg-slate-50">
                                        ...
                                        <td className="px-2 py-4 text-center">{formatJH(project.internalWorkloadConsumed)}</td>
                                        <td className="px-2 py-4 text-center text-slate-400">{formatJH(project.internalWorkloadEngaged)}</td>
                                        <td className="px-2 py-4 text-center">{formatJH(project.externalWorkloadConsumed)}</td>
                                        <td className="px-2 py-4 text-center text-slate-400">{formatJH(project.externalWorkloadEngaged)}</td>
                                    </tr>
                                )})}
                            </tbody>
                        </table>
